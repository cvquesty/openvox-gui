...
function UserManagerTab() {
  ...
  return (
    <Stack>
      {/* Authentication panel — moved here from Application tab */}
      <Card withBorder shadow="sm">
        <Text fw={700} mb="sm">Authentication</Text>
        <Group>
          <Text size="sm" c="dimmed">Current Backend:</Text>
          <Badge color={appData?.auth_backend === 'none' ? 'yellow' : 'green'} size="lg">
            {appData?.auth_backend || 'none'}
          </Badge>
        </Group>
          ...
          <Card withBorder shadow="sm" padding="lg">
            ...
            <Stack gap="sm">
              ...
              <Button leftSection={<IconPlus size={16} />} onClick={handleAddUser}
                loading={addLoading} disabled={!newUsername || !newPassword}>
                Create User
              </Button>
            </Stack>
          </Card>
          ...
          <Table.Tbody>
            {userList.map((u) => (
              <Table.Tr key={u.username}>
                ...
                <Table.Td>
                  <Badge color={roleBadgeColor[u.role] || 'gray'} variant="light">{u.role}</Badge>
                </Table.Td>
                <Table.Td>
                  <Group gap="xs" justify="flex-end">
                    ...
                    {u.username !== currentUser?.username && (
                      <Tooltip label="Delete user">
                        <ActionIcon variant="subtle" color="red" onClick={() => handleDeleteUser(u.username)}>
                          <IconTrash size={16} />
                        </ActionIcon>
                      </Tooltip>
                    )}
