#!/bin/bash
###############################################################################
# OpenVox GUI Remote Update Script v1.1.0
#
...
echo -e "${BOLD}OpenVox GUI Remote Deployment v1.1.0${NC}"
...
if [ "$UNATTENDED" = true ]; then
    ssh -t "${REMOTE_USER}@${REMOTE_HOST}" "cd ${INSTALL_DIR} && sudo bash /tmp/openvox-deploy.sh --yes"
else
    ssh -t "${REMOTE_USER}@${REMOTE_HOST}" "cd ${INSTALL_DIR} && sudo bash /tmp/openvox-deploy.sh"
fi
