...
# 5f. Write the full current sudoers rules (unattended, matching docs/SUDOERS.md and install.sh).
# This ensures the test/production system always gets the correct explicit rules
# for puppet, bolt (via the full binary), etc., without manual intervention after
# install or update/deploy.
SUDOERS_FILE=/etc/sudoers.d/openvox-gui
cat > "$SUDOERS_FILE" << SUDOEOF
# OpenVox GUI — sudoers configuration (explicit, no wildcards)
# Generated by deploy.sh. This file grants the ${SERVICE_USER} user passwordless
# sudo access **only** to the specific commands listed below.
#
# NOTE: All rules are explicit. No trailing * wildcards are used.
...
# This permits the GUI (running as the ${SERVICE_USER} user) to execute
...
echo "  wrote full explicit /etc/sudoers.d/openvox-gui (matching docs/SUDOERS.md)"
